...
Mailer.send = function (options, cb) { // eslint-disable-line
  ...
  if (connector.sendgrid) {
    ...
    if (R.is(Array, options.to) && options.to.length!==0) {
    ...
    if (R.is(Array, sendgridEmail.personalizations) && sendgridEmail.personalizations.length > 0) {
      if (options.cc) {
        for(let cc of options.cc){
          sendgridEmail.personalizations[0].addCc(typeof cc === "string" ? {email:cc} : cc);
        }
      }
      if (options.bcc) {
        for(let bcc of options.bcc){
          sendgridEmail.personalizations[0].addBcc(typeof bcc === "string" ? {email:bcc} : bcc);
        }
      }
      if(sendgridMessage.to.length>0){
        for(let to of sendgridMessage.to){
            sendgridEmail.personalizations[0].addTo(to)
        }
      }
    }
